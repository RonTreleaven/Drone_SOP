...
pm = folium.Marker([pilot_lat, pilot_lon],
        ...
        icon=folium.Icon(color="pink", icon="user", prefix="fa")).add_to(m)
pm_js = pm.get_name()
...
obstacle_layer = folium.FeatureGroup(name="Obstacles")
for notam in notams_data:
    for coord in notam.get("coordinates_dd", []):
        lat, lon = map(float, coord.split(","))
        popup_content = f"<b>Obstacle Alert</b><br>{notam['raw'].replace(chr(10), '<br>')}"
        folium.Marker(
            [lat, lon],
            tooltip="Obstacle",
            popup=popup_content,
