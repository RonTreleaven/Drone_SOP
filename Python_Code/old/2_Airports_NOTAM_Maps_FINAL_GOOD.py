...
# === Pilot input (safe fallback) ===
try:
    pilot_lat = float(input("Pilot latitude (default 44.0): ") or 44.0)
    pilot_lon = float(input("Pilot longitude (default -79.0): ") or -79.0)
except Exception:
    pilot_lat, pilot_lon = 44.0, -79.0
print(f"Pilot location set to: {pilot_lat}, {pilot_lon}")

nm_to_m = 1852
zoom_init = 8

# === Load local airports.csv ===
if not os.path.exists("airports.csv"):
    raise SystemExit("Missing required file: airports.csv")
...
# === Pilot marker and rings ===
folium.Marker([pilot_lat, pilot_lon], tooltip="Pilot", popup="Pilot Location",
...
for notam in notams_data:
    ...
    for coord in notam["coordinates_dd"]:
        ...
        folium.Marker([lat, lon], tooltip="Obstacle",
...
m.get_root().html.add_child(folium.Element(
    '<script>if (!window._all_layers) window._all_layers = {};'
    'window._all_layers["NOTAMS"] = ' + obstacle_layer.get_name() + ';</script>'
))

# === Sidebar for filters ===
prov_opts = ''.join(f'<option value="{p}">{p}</option>' for p in province_names)
type_opts = ''.join(f'<option value="{t}">{t}</option>' for t in airport_types)

sidebar_html = '''
<div id="sidebar" style="position:fixed;top:10px;left:10px;z-index:9999;background:white;
    padding:10px;border:1px solid #ccc;max-height:90%;overflow:auto">
  <h4>Filters</h4>
  <label>Province:</label><br>
  <select id="provSelect" multiple size="5">{}</select><br><br>
  <label>Airport Types:</label><br>
  <select id="typeSelect" multiple size="5">{}</select><br><br>
  <button onclick="applyFilter()">Apply Filter</button>
</div>
<script>
function applyFilter() {{
  // Always show obstacle layer too
  if (window._all_layers && window._all_layers["NOTAMS"]) {
    window._map.addLayer(window._all_layers["NOTAMS"]);
  }
  ...
  for (let id in layers) {{
    let obj = layers[id];
    let [prov, type] = id.split(":");
    if (provs.includes(prov) && types.includes(type)) {{
      obj.addTo(window._map);
    }} else {{
      window._map.removeLayer(obj);
    }}
  }}
}}
</script>
'''.format(prov_opts, type_opts)

m.get_root().html.add_child(folium.Element(sidebar_html))
m.save("Airports_NOTAMS.html")
print("✅ Map saved to Airports_NOTAMS.html")