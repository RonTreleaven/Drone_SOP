...
pilot_lat = 44.0
pilot_lon = -79.0
print("Pilot location set to: 44.0, -79.0")
nm_to_m = 1852
zoom_init = 8
...
<div id="sidebar" style="position:fixed;top:10px;left:10px;z-index:9999;background:white;padding:10px;border:1px solid #ccc;max-height:90%;overflow:auto">
  ...
  <select id="provSelect" multiple size="5">{''.join(f'<option value="{p}">{p}</option>' for p in province_names)}</select><br><br>
  <label>Airport Types:</label><br>
  <select id="typeSelect" multiple size="5">{''.join(f'<option value="{t}">{t}</option>' for t in airport_types)}</select><br><br>
