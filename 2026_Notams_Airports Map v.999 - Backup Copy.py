...
def build_map(pilot_lat, pilot_lon, notams_data, generated_at=None):
    ...
    for notam in notams_data:
        ...
        for coord in coords:
            ...
            if haversine_km(pilot_lat, pilot_lon, lat, lon) <= NOTAM_RADIUS_KM:
                shown += 1
            ...
            obstacle_entries_js.append(
                f"{{marker:{marker.get_name()},lat:{lat},lon:{lon}}}"
            )
    ...
    # 7) Inject widget for pilot drag/manual update and dynamic NOTAM filter
    widget_js = f"""
<script>
window.onload = function() {{
  ...
  var obstacles = [{','.join(obstacle_entries_js)}];
  ...
  function refreshObstacles(ll) {{
      for (var i = 0; i < obstacles.length; i++) {{
          var obs = obstacles[i];
          var km = haversineKm(ll.lat, ll.lng, obs.lat, obs.lon);
          if (km <= radiusKm) {{
              if (!obstacleLayer.hasLayer(obs.marker)) {{ obs.marker.addTo(obstacleLayer); }}
          }} else {{
              if (obstacleLayer.hasLayer(obs.marker)) {{ obstacleLayer.removeLayer(obs.marker); }}
          }}
      }}
  }}
