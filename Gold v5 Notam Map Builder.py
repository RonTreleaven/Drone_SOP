...
#
# version 5.0 to create NotamsMap.html (v5)
...
COORD_DECIMALS = 6
LIVE_MAP_FILENAME = "NotamsMap.html"
DEFAULT_V5_OUTPUT_FILENAME = "NotamsMapv5.html"
OVERWRITE_CONFIRM_TOKEN = "OVERWRITE_NOTAMSMAP"
SCRIPT_VERSION = "v5.0.0"
SCRIPT_RELEASE_DATE = "2026-05-30"
SCRIPT_NAME = "Gold v5 Notam Map Builder.py"
...
DEFAULT_BASEMAP_KEY = "google_terrain"
BASEMAP_OPTIONS = {
    "cartodb_positron": {
...
def build_map(
    ...
    m.get_root().html.add_child(
        folium.Element(
            ...
            ".pilot-flyout.collapsed{display:none!important;}" 
...
<style>
  ...
  .map-layers-control.leaflet-control-layers-expanded {
    ...
    .leaflet-bottom.leaflet-left .map-layers-control.leaflet-control-layers {
        margin-bottom: 10px;
        margin-left: 10px;
        z-index: 1306;
    }
    .leaflet-bottom.leaflet-left .map-layers-control.leaflet-control-layers-expanded {
                position: absolute;
                left: 0;
                bottom: 0;
  ...
  .map-layers-control .leaflet-control-layers-list {
    ...
    @media (max-width: 768px) {
        ...
        .leaflet-bottom.leaflet-left .map-layers-control .leaflet-control-layers-list {
            max-height: 30vh;
            overflow-y: auto;
        }
    }
...
<style>
    .airport-type-control.leaflet-control-layers {
        background: transparent !important;
        border: none !important;
        box-shadow: none !important;
    }
    .airport-type-control .leaflet-control-layers-toggle {
    ...
    .airport-type-control .leaflet-control-layers-toggle::before {
        content: '\\f072';
    ...
    .airport-type-control.leaflet-control-layers-expanded {
        background: #ffffff !important;
        border: 1px solid #d8e2ee !important;
        box-shadow: 0 2px 6px rgba(0,0,0,0.2) !important;
        border-radius: 8px;
    ...
    .airport-type-control .leaflet-control-layers-list {
        max-height: 56vh;
        overflow-y: auto;
        touch-action: pan-y;
        overscroll-behavior: contain;
    }
    .airport-type-control .airport-type-close-btn {
        display: none;
        margin-top: 8px;
        width: 100%;
        border: 1px solid #d8e2ee;
    ...
    .airport-type-control.leaflet-control-layers-expanded .airport-type-close-btn {
        display: block;
    }

    .pilot-flyout.pilot-flyout-v2 {
        width: 344px;
    ...
    .pilot-flyout-v2 .drawer-section {
        margin-top: 8px;
        padding-top: 7px;
        border-top: 1px solid #d8e2ee;
    }
    .pilot-flyout-v2 .drawer-global-actions {
        margin-bottom: 6px;
    }
    .pilot-flyout-v2 .drawer-global-btn {
    ...
    .pilot-flyout-v2 .drawer-global-btn:hover {
        background: #edf4fb;
    }
    .pilot-flyout-v2 .drawer-section-header {
        display: flex;
        align-items: center;
        justify-content: space-between;
        gap: 8px;
    ...
    .pilot-flyout-v2 .drawer-section-header .drawer-section-title {
        margin: 0;
        cursor: pointer;
    }
    .pilot-flyout-v2 .drawer-section-toggle {
        border: 1px solid #9eb9d2;
        background: #ffffff;
        color: #294766;
        border-radius: 999px;
    ...
    .pilot-flyout-v2 .drawer-section-toggle:hover {
        background: #edf4fb;
    }
    .pilot-flyout-v2 .drawer-section-body {
        display: block;
    }
    .pilot-flyout-v2 .drawer-section.is-collapsed .drawer-section-body {
        display: none;
    }
    .pilot-flyout-v2 .drawer-section:first-of-type {
        margin-top: 2px;
        padding-top: 0;
        border-top: none;
    }
    .pilot-flyout-v2 .drawer-section-title {
        font-size: 12px;
        font-weight: 700;
        color: #1f3b63;
        margin-bottom: 5px;
    }
    .pilot-flyout-v2 .pilot-header-row {
        display: flex;
        align-items: center;
        justify-content: space-between;
        gap: 8px;
    ...
    .pilot-flyout-v2 .pilot-header-title {
        font-size: 12px;
        font-weight: 700;
        color: #1f3b63;
        letter-spacing: 0.01em;
    }
    .pilot-flyout-v2 .pilot-icon-btn {
        width: 34px;
        height: 34px;
    ...
    .pilot-flyout-v2 .pilot-icon-btn:hover { background: #f3f8ff; }
    .pilot-flyout-v2 .latlon-grid {
        display: grid;
        grid-template-columns: minmax(0, 1fr) minmax(0, 1fr) auto;
        gap: 6px;
        align-items: start;
    }
    .pilot-flyout-v2 .coord-field {
        display: flex;
        flex-direction: column;
        gap: 1px;
        min-width: 0;
    }
    .pilot-flyout-v2 .coord-field span {
        font-size: 11px;
        font-weight: 700;
        color: #435b78;
    }
    .pilot-flyout-v2 .coord-field input {
        width: 100%;
        max-width: none;
        min-height: 34px;
    ...
    .pilot-flyout-v2 .latlon-quick-actions {
        display: flex;
        flex-direction: column;
        align-items: flex-start;
        gap: 6px;
    }
    .pilot-flyout-v2 #clearPilotLatLon {
        width: 34px;
        flex: 0 0 34px;
        border-color: #d7a4a1;
        background: #fff1ef;
    ...
    .pilot-flyout-v2 .dd-hint {
        margin: 0 0 3px;
        font-size: 11px;
        color: #5e6c7d;
    }
    .pilot-flyout-v2 .action-grid {
        margin-top: 8px;
        gap: 6px;
    }
    .pilot-flyout-v2 .action-grid button {
        height: 32px;
        font-size: 11px;
        line-height: 1.1;
        border-radius: 7px;
    }
    .pilot-flyout-v2 #refreshFirCacheBtn.refresh-fir-stale {
        background: #c62828;
        border-color: #8f1d1d;
        color: #fff;
        font-weight: 700;
    }
    .pilot-flyout-v2 #refreshFirCacheBtn.refresh-fir-stale:hover {
        background: #b71c1c;
    }
    .pilot-flyout-v2 .radius-row {
        margin-top: 8px;
        display: grid;
    ...
    .pilot-flyout-v2 .radius-row .radius-label {
        font-size: 11px;
        font-weight: 700;
        color: #435b78;
        white-space: nowrap;
    }
    .pilot-flyout-v2 .radius-row input {
        width: 100%;
        max-width: none;
        min-height: 30px;
        border: 1px solid #b8cbe2;
    ...
    .pilot-flyout-v2 .radius-row button {
        min-height: 30px;
        font-size: 11px;
        border-radius: 7px;
    }
    .pilot-flyout-v2 .pilot-status-block {
        margin-top: 8px;
        padding-top: 6px;
        border-top: 1px solid #d8e2ee;
    }
    .pilot-flyout-v2 .controls-meta {
        margin-top: 4px;
        line-height: 1.3;
        font-size: 11px;
    }
    .pilot-flyout-v2 #currentPilotText { color: #1d4f83; }

  @media (max-width: 768px) {
        .airport-circles-flyout {
            width: 206px;
            left: 40px;
            max-height: 46vh;
        }
    ...
    .pilot-flyout .action-grid button {
      ...
      font-size: 11px;
        ...
        .pilot-flyout-v2 .latlon-grid { gap: 4px; align-items: start; }
        .pilot-flyout-v2 .coord-field input { min-height: 30px; }
        .pilot-flyout-v2 .latlon-quick-actions { gap: 3px; }
        .pilot-flyout-v2 .pilot-icon-btn { height: 30px; }
        .pilot-flyout-v2 .dd-hint { margin: 0 0 2px; }
        .pilot-flyout-v2 .action-grid { margin-top: 6px; gap: 4px; }
        .pilot-flyout-v2 .action-grid button { height: 30px; }
        .pilot-flyout-v2 .radius-row { margin-top: 6px; gap: 4px; }
        .pilot-flyout-v2 .radius-row input,
        .pilot-flyout-v2 .radius-row button { min-height: 28px; }
        .pilot-flyout-v2 .pilot-status-block { margin-top: 6px; padding-top: 5px; }
        .pilot-flyout-v2 .controls-meta { margin-top: 2px; line-height: 1.25; }
  }
</style>
<style>
  .pilot-input {
  ...
  .center-map-control {
    ...
    .airport-circles-flyout.collapsed {
        display: none;
    }
    .airport-circles-head {
        display: flex;
        align-items: center;
        justify-content: space-between;
        gap: 8px;
    ...
    .airport-circles-title {
        font-weight: 700;
        color: #1f3b63;
    }
    .airport-circles-close {
        border: 1px solid #9bb6da;
        border-radius: 6px;
        background: #fff;
        color: #1f3b63;
    ...
    .airport-circles-row-label {
        flex: 1;
        color: #1f3b63;
    }
    .airport-circles-row-status {
        font-weight: 700;
        color: #4a5568;
    }
    .airport-circles-hint {
        margin-top: 4px;
        font-size: 11px;
        color: #6b7280;
    }
...
window.onload = function() {{
  ...
  var c100 = {c100_js};
    ...
    var setUnifiedDrawerOpen = null;
    var setAirportCircleFlyoutOpen = null;
    var setNotamsFlyoutOpen = null;
    var allNotamDetailsShown = false;
    ...
    var refreshFirStaleMs = 24 * 60 * 60 * 1000;
    ...
    if (typeof map.options.tapTolerance === 'number' && map.options.tapTolerance < 22) {{
        ...
        var basemapControl = L.control.layers(basemapLayers, null, {{ position: 'bottomleft', collapsed: true }});
  ...
  function getPilotLocationFromQuery() {{
      try {{
          var params = new URLSearchParams(window.location.search || '');

          var centerRaw = (params.get('center') || '').trim();
          if (centerRaw) {{
              var centerPair = splitCoordinatePair(centerRaw);
              if (centerPair) {{
                  var centerLl = L.latLng(centerPair.lat, centerPair.lon);
                  if (hasUsablePilotLocation(centerLl)) {{
                      return centerLl;
                  }}
              }}
          }}

          var pilotRaw = (params.get('pilot') || '').trim();
          if (pilotRaw) {{
              var pilotPair = splitCoordinatePair(pilotRaw);
              if (pilotPair) {{
                  var pilotLl = L.latLng(pilotPair.lat, pilotPair.lon);
                  if (hasUsablePilotLocation(pilotLl)) {{
                      return pilotLl;
                  }}
              }}
          }}

          var pilotLatRaw = params.get('pilotLat');
          var pilotLonRaw = params.get('pilotLon');
          if (pilotLatRaw != null && pilotLonRaw != null) {{
              var pilotLat = Number(pilotLatRaw);
              var pilotLon = Number(pilotLonRaw);
              if (isFinite(pilotLat) && isFinite(pilotLon)) {{
                  var explicitPilotLl = L.latLng(pilotLat, pilotLon);
                  if (hasUsablePilotLocation(explicitPilotLl)) {{
                      return explicitPilotLl;
                  }}
              }}
          }}
      }} catch (err) {{
          console.warn('[NOTAM MAP] unable to parse pilot location from URL', err);
      }}
      return null;
  }}
  ...
  function getLinkedRealtimeNotamsFromQuery() {{
      ...
      var hasExplicitFocusCoord = false;
      var latRaw = params.get('lat') || params.get('latitude');
      var lonRaw = params.get('lon') || params.get('lng') || params.get('longitude');
      if (latRaw != null && lonRaw != null) {{
          var lat = Number(latRaw);
          var lon = Number(lonRaw);
          hasExplicitFocusCoord = isFinite(lat) && isFinite(lon) && lat >= -90 && lat <= 90 && lon >= -180 && lon <= 180;
      }}
      if (payloadKey) {{
      ...
      if (src === 'fetchnotams' && !hasExplicitFocusCoord) {{
  ...
  function getLatestKnownLivePayloadCreatedAt() {{
      var latestCreatedAt = Number(linkedRealtimePayloadCreatedAt) || 0;
      try {{
          var startupRaw = localStorage.getItem(liveUnfilteredPayloadStorageKey);
          if (startupRaw) {{
              var startupParsed = JSON.parse(startupRaw);
              var startupCreatedAt = Number(startupParsed && startupParsed.createdAt) || 0;
              var startupRecords = Array.isArray(startupParsed && startupParsed.records) ? startupParsed.records : [];
              if (startupCreatedAt && startupRecords.length) {{
                  latestCreatedAt = Math.max(latestCreatedAt, startupCreatedAt);
              }}
          }}
  ...
  function updateRefreshFirButtonState() {{
      var refreshBtn = document.getElementById('refreshFirCacheBtn');
      if (!refreshBtn) {{
          return;
      }}

      var createdAt = getLatestKnownLivePayloadCreatedAt();
      refreshBtn.classList.remove('refresh-fir-stale');

      if (!createdAt) {{
          refreshBtn.title = 'Refresh FIR (no cached live payload found)';
          return;
      }}

      var ageMs = Math.max(0, Date.now() - createdAt);
      if (ageMs > refreshFirStaleMs) {{
          refreshBtn.classList.add('refresh-fir-stale');
      }}

      var ageHours = Math.floor(ageMs / (60 * 60 * 1000));
      if (ageHours < 1) {{
          var ageMins = Math.max(1, Math.floor(ageMs / 60000));
          refreshBtn.title = 'Refresh FIR (cache age: ' + ageMins + ' min)';
      }} else {{
          refreshBtn.title = 'Refresh FIR (cache age: ' + ageHours + ' hr)';
      }}
  }}
  ...
  function updatePilotDisplay(ll) {{
      ...
      var ddText = document.getElementById('decimalDegreesText');
      ...
      if (ddText) {{
          ddText.textContent = 'Decimal Degrees: ' + formatCoord(ll.lat) + ', ' + formatCoord(ll.lng);
      }}
      if (pilotText) {{
  ...
  function updateAirportsToggleLabel() {{
      ...
      var provinceRow = document.getElementById('airportProvinceToggleRow');
      var provinceStatus = document.getElementById('airportProvinceStatus');
      ...
      if (provinceStatus) {{
          provinceStatus.textContent = active ? 'On' : 'Off';
      }}
      if (provinceRow) {{
          provinceRow.style.opacity = active ? '1' : '0.55';
      }}
  }}
  ...
  function updateNotamsToggleVisual() {{
      ...
      if (typeof updateNotamsFlyoutStatus === 'function') {{
          updateNotamsFlyoutStatus();
      }}
  }}

  function updateNotamsFlyoutStatus() {{
      var status = document.getElementById('notamsFlyoutStatus');
      var toggleBtn = document.getElementById('notamsFlyoutToggleBtn');
      var detailBtn = document.getElementById('showAllNotamDetailBtn');
      var count = window._notamObstacleCount || 0;
      if (status) {{
          status.textContent = 'In-scope markers: ' + String(count);
      }}
      if (toggleBtn) {{
          toggleBtn.textContent = notamsVisible ? 'Hide NOTAM markers' : 'Show NOTAM markers';
      }}
      if (detailBtn) {{
          detailBtn.textContent = allNotamDetailsShown ? 'Hide all NOTAM detail' : 'Show all NOTAM detail';
      }}
  }}

  function toggleAllNotamDetails() {{
      if (!obstacleMarkers || obstacleMarkers.length === 0) {{
          alert('No in-scope NOTAM markers to show.');
          return;
      }}

      if (!notamsVisible) {{
          setNotamsVisible(true);
      }}

      if (!allNotamDetailsShown) {{
          if (obstacleMarkers.length > 40) {{
              var proceed = window.confirm('Open detail popups for ' + obstacleMarkers.length + ' NOTAM markers?');
              if (!proceed) {{
                  return;
              }}
          }}
          ...
          for (var i = 0; i < obstacleMarkers.length; i++) {{
              var mkOpen = obstacleMarkers[i];
              if (!mkOpen || !mkOpen.getPopup) continue;
              var popupOpen = mkOpen.getPopup();
              if (!popupOpen) continue;
              popupOpen.options.autoClose = false;
              popupOpen.options.closeOnClick = false;
              popupOpen.options.maxWidth = 420;
              mkOpen.openPopup();
          }}
          allNotamDetailsShown = true;
      }} else {{
          for (var j = 0; j < obstacleMarkers.length; j++) {{
              var mkClose = obstacleMarkers[j];
              if (!mkClose || !mkClose.closePopup) continue;
              mkClose.closePopup();
          }}
          if (map && map.closePopup) {{
              map.closePopup();
          }}
          allNotamDetailsShown = false;
      }}
  ...
  // Enrich airport markers: compact hover distance/DD and concise click links
  function enrichAirportMarkersWithDistance(pLat, pLon) {{
      ...
      function extractAirportCode(htmlText) {{
          if (!htmlText) return null;
          var plain = String(htmlText).replace(/<[^>]*>/g, ' ').replace(/\\s+/g, ' ').trim();
  ...
  var pilotToggleCtl = L.control({{position:'topleft'}});
  pilotToggleCtl.onAdd = function() {{
      var d = L.DomUtil.create('div', 'left-mini-toggle-control');
      d.id = 'pilotToggleCtl';
      var btn = L.DomUtil.create('a', 'left-mini-toggle-btn', d);
      btn.id = 'pilotToggleBtn';
      btn.href = '#';
      btn.title = 'Pilot controls';
      btn.setAttribute('aria-label', 'Pilot controls');
      ...
      L.DomEvent.on(btn, 'click', function(ev) {{
          L.DomEvent.stop(ev);
          if (typeof setUnifiedDrawerOpen === 'function') {{
              setUnifiedDrawerOpen(true, 'pilot');
          }}
      }});
      return d;
  }};
  pilotToggleCtl.addTo(map);

  basemapControl.addTo(map);
  var basemapCtlContainer = basemapControl.getContainer ? basemapControl.getContainer() : null;
  var basemapViewportFitTimer = null;

  function fitBasemapControlToViewport() {{
      if (!basemapCtlContainer) return;
      basemapCtlContainer.style.transform = '';
      if (!basemapCtlContainer.classList.contains('leaflet-control-layers-expanded')) return;

      var rect = basemapCtlContainer.getBoundingClientRect();
      var topOverflow = 8 - rect.top;
      var bottomOverflow = rect.bottom - (window.innerHeight - 8);
      var shiftY = 0;
      if (topOverflow > 0) shiftY += topOverflow;
      if (bottomOverflow > 0) shiftY -= bottomOverflow;
      if (shiftY !== 0) {{
          basemapCtlContainer.style.transform = 'translateY(' + String(Math.round(shiftY)) + 'px)';
      }}
  }}

  function scheduleBasemapViewportFit() {{
      if (basemapViewportFitTimer) {{
          window.clearTimeout(basemapViewportFitTimer);
      }}
      basemapViewportFitTimer = window.setTimeout(fitBasemapControlToViewport, 0);
  }}
  ...
  if (basemapCtlContainer) {{
      ...
      if (baseToggle) {{
          ...
          L.DomEvent.on(baseToggle, 'click', function() {{
              scheduleBasemapViewportFit();
          }});
      }}
      ...
      if (window && window.addEventListener) {{
          window.addEventListener('resize', scheduleBasemapViewportFit);
      }}
  }}

  (function setupAirportTypeControl() {{
      var airportTypeCtlContainer = document.querySelector('.leaflet-top.leaflet-right .leaflet-control-layers');
      if (!airportTypeCtlContainer) {{
          return;
      }}

      airportTypeCtlContainer.classList.add('airport-type-control');
      airportTypeCtlContainer.title = 'Airport / Type Layers';
      L.DomEvent.disableClickPropagation(airportTypeCtlContainer);
      L.DomEvent.disableScrollPropagation(airportTypeCtlContainer);

      var airportTypeToggle = airportTypeCtlContainer.querySelector('.leaflet-control-layers-toggle');
      if (airportTypeToggle) {{
          airportTypeToggle.title = 'Airport / Type Layers';
          airportTypeToggle.setAttribute('aria-label', 'Airport / Type Layers');
          L.DomEvent.on(airportTypeToggle, 'click', function(ev) {{
              L.DomEvent.stop(ev);
              airportTypeCtlContainer.classList.toggle('leaflet-control-layers-expanded');
          }});
      }}

      var airportTypeList = airportTypeCtlContainer.querySelector('.leaflet-control-layers-list');
      if (airportTypeList) {{
          L.DomEvent.on(airportTypeList, 'touchstart', L.DomEvent.stopPropagation);
          L.DomEvent.on(airportTypeList, 'touchmove', L.DomEvent.stopPropagation);
      }}

      if (!airportTypeCtlContainer.querySelector('.airport-type-close-btn')) {{
          var airportTypeCloseBtn = L.DomUtil.create('button', 'airport-type-close-btn', airportTypeCtlContainer);
          airportTypeCloseBtn.type = 'button';
          airportTypeCloseBtn.textContent = 'Close';
          airportTypeCloseBtn.setAttribute('aria-label', 'Close Airport / Type layers panel');
          airportTypeCloseBtn.title = 'Close';
          L.DomEvent.on(airportTypeCloseBtn, 'click', function(ev) {{
              L.DomEvent.stop(ev);
              airportTypeCtlContainer.classList.remove('leaflet-control-layers-expanded');
          }});
      }}

      map.on('click', function() {{
          if (airportTypeCtlContainer.classList.contains('leaflet-control-layers-expanded')) {{
              airportTypeCtlContainer.classList.remove('leaflet-control-layers-expanded');
          }}
      }});
  }})();
  ...
  ctl.onAdd = function() {{
      ...
      d.style.left = '46px';
      d.style.top = '0px';
      d.innerHTML = '<div id="pilotFlyout" class="pilot-flyout pilot-flyout-v2"><div class="pilot-header-row"><div class="pilot-header-title">Operations Drawer</div><button id="pilotFlyoutClose" class="pilot-icon-btn" title="Close Drawer"><i class="fa-solid fa-xmark"></i></button></div><section id="drawerNotamsSection" class="drawer-section"><div class="drawer-section-title">NOTAM Tools</div><button type="button" id="notamsFlyoutToggleBtn" class="notams-flyout-btn">Hide NOTAM markers</button><button type="button" id="showAllNotamDetailBtn" class="notams-flyout-btn">Show all NOTAM detail</button><div id="notamsFlyoutStatus" class="notams-flyout-meta">In-scope markers: 0</div></section><section id="drawerAirportSection" class="drawer-section"><div class="drawer-section-title">Airport Circle Layers</div><div id="airportCircleRows"></div><div class="airport-circles-hint">Tap a row to toggle</div></section><section id="drawerPilotSection" class="drawer-section"><div class="drawer-section-title">Pilot Controls</div><div id="decimalDegreesText" class="controls-meta dd-hint">Decimal Degrees: --, --</div><div class="latlon-grid"><label class="coord-field"><span>Lat</span><input id="pLat" size="12" maxlength="12" placeholder="43.000000"></label><label class="coord-field"><span>Lon</span><input id="pLon" size="13" maxlength="13" placeholder="-79.000000"></label><div class="latlon-quick-actions"><button id="clearPilotLatLon" class="pilot-icon-btn" title="Clear Lat/Lon"><i class="fa-solid fa-xmark"></i></button><button id="upd" class="pilot-icon-btn" title="Set Lat/Lon"><i class="fa-solid fa-check"></i></button></div></div><div class="action-grid"><button id="getPilotLoc">Get Pilot Location</button><button id="sourceBtn">Switch Filter Group</button><button id="openLiveFetch">Open FetchNotams</button><button id="refreshFirCacheBtn">Refresh FIR</button></div><div class="radius-row"><span class="radius-label">Radius km</span><input id="radiusKmInput" size="6" maxlength="6" placeholder="10"><button id="setRadiusBtn">Change Radius</button></div><div class="pilot-status-block"><div id="currentPilotText" class="controls-meta status-line">Current Pilot Location: --, --</div><div id="notamSourceText" class="controls-meta status-line">NOTAM Source: RAW</div><div id="livePayloadText" class="controls-meta status-line">Live payload: --</div><div id="currentBasemapText" class="controls-meta status-line">Basemap: --</div><div id="radiusInfoText" class="controls-meta status-line">NOTAM Radius: 10 km</div><div id="obstacleCountText" class="controls-meta status-line">Obstacles (10 km): --</div></div><div id="refreshFirPanel" style="display:none;padding:5px 2px 2px 2px;border-top:1px solid #ccc;margin-top:5px;"><div style="font-size:0.82em;font-weight:600;margin-bottom:3px;color:#555;">Refresh NOTAMs Cache</div><div style="display:flex;gap:4px;align-items:center;"><span style="font-size:0.8em;">FIR:</span><input id="refreshFirInput" size="5" maxlength="4" placeholder="CZYZ" style="width:50px;font-size:0.8em;padding:2px 3px;border:1px solid #aaa;border-radius:3px;"><button id="refreshFirFetchBtn" style="font-size:0.8em;padding:2px 7px;background:#2d6a2d;color:#fff;border:none;border-radius:3px;cursor:pointer;">Fetch</button><button id="refreshFirCancelBtn" style="font-size:0.8em;padding:2px 6px;background:#888;color:#fff;border:none;border-radius:3px;cursor:pointer;">&#x2715;</button></div><div id="refreshFirStatus" style="font-size:0.78em;margin-top:4px;color:#555;min-height:1.1em;"></div></div></section></div>';
      return d;
  }};
  ctl.addTo(map);
  ...
  function setPilotFlyoutCollapsed(collapsed) {{
      ...
      var pilotToggleBtn = document.getElementById('pilotToggleBtn');
      var notamsToggleBtn = document.getElementById('notamsToggleBtn');
      var airportsToggleBtn = document.getElementById('airportsToggleBtn');
      if (!flyout) {{
          return;
      }}
      flyout.classList.toggle('collapsed', !!collapsed);
      var expanded = String(!flyout.classList.contains('collapsed'));
      if (pilotToggleBtn) pilotToggleBtn.setAttribute('aria-expanded', expanded);
      if (notamsToggleBtn) notamsToggleBtn.setAttribute('aria-expanded', expanded);
      if (airportsToggleBtn) airportsToggleBtn.setAttribute('aria-expanded', expanded);
  }}

  function drawerSectionDefaults(sectionId) {{
      if (sectionId === 'drawerNotamsSection') return true;
      if (sectionId === 'drawerAirportSection') return true;
      return false;
  }}

  function updateDrawerGlobalToggleButton() {{
      var btn = document.getElementById('toggleAllDrawerSectionsBtn');
      if (!btn) return;
      var sections = Array.from(document.querySelectorAll('#pilotFlyout .drawer-section'));
      if (!sections.length) return;
      var allCollapsed = sections.every(function(section) {{
          return section.classList.contains('is-collapsed');
      }});
      btn.textContent = allCollapsed ? 'Show All' : 'Hide All';
      btn.setAttribute('aria-label', allCollapsed ? 'Show all drawer sections' : 'Hide all drawer sections');
  }}

  function setAllDrawerSectionsCollapsed(collapsed) {{
      var sections = Array.from(document.querySelectorAll('#pilotFlyout .drawer-section'));
      sections.forEach(function(section) {{
          if (typeof section._setCollapsedState === 'function') {{
              section._setCollapsedState(!!collapsed);
          }}
      }});
      updateDrawerGlobalToggleButton();
  }}

  function ensureDrawerSectionToggles() {{
      var flyout = document.getElementById('pilotFlyout');
      if (!flyout) return;

      var headerRow = flyout.querySelector('.pilot-header-row');
      if (headerRow && !document.getElementById('toggleAllDrawerSectionsBtn')) {{
          var globalActions = document.createElement('div');
          globalActions.className = 'drawer-global-actions';
          var globalBtn = document.createElement('button');
          globalBtn.id = 'toggleAllDrawerSectionsBtn';
          globalBtn.type = 'button';
          globalBtn.className = 'drawer-global-btn';
          globalBtn.textContent = 'Hide All';
          globalBtn.addEventListener('click', function() {{
              var sections = Array.from(document.querySelectorAll('#pilotFlyout .drawer-section'));
              var allCollapsed = sections.length && sections.every(function(section) {{
                  return section.classList.contains('is-collapsed');
              }});
              setAllDrawerSectionsCollapsed(!allCollapsed);
          }});
          globalActions.appendChild(globalBtn);
          headerRow.insertAdjacentElement('afterend', globalActions);
      }}

      var sections = Array.from(flyout.querySelectorAll('.drawer-section'));
      sections.forEach(function(section) {{
          if (section.dataset.collapsibleReady === '1') return;

          var titleEl = section.querySelector(':scope > .drawer-section-title');
          if (!titleEl) return;
          ...
          title.textContent = String(titleEl.textContent || '').trim();
          title.tabIndex = 0;
          title.setAttribute('role', 'button');
          title.setAttribute('aria-label', 'Toggle section');

          var toggle = document.createElement('button');
          toggle.type = 'button';
          toggle.className = 'drawer-section-toggle';

          var body = document.createElement('div');
          body.className = 'drawer-section-body';

          var node = titleEl.nextSibling;
          while (node) {{
          ...
          section._setCollapsedState = setCollapsed;
          ...
          setCollapsed(drawerSectionDefaults(section.id));
  ...
  function focusDrawerSection(sectionKey) {{
      ensureDrawerSectionToggles();
      var target = null;
      if (sectionKey === 'notams') target = document.getElementById('drawerNotamsSection');
      if (sectionKey === 'airport') target = document.getElementById('drawerAirportSection');
      if (sectionKey === 'pilot') target = document.getElementById('drawerPilotSection');
      if (target && typeof target._setCollapsedState === 'function') {{
          target._setCollapsedState(false);
      }}
      if (!target || !target.scrollIntoView) return;
      target.scrollIntoView({{ behavior: 'smooth', block: 'nearest' }});
  }}

  setUnifiedDrawerOpen = function(forceOpen, sectionKey) {{
      var flyout = document.getElementById('pilotFlyout');
      if (!flyout) return;
      ensureDrawerSectionToggles();
      if (typeof forceOpen === 'boolean') {{
          setPilotFlyoutCollapsed(!forceOpen);
      }} else {{
          setPilotFlyoutCollapsed(!flyout.classList.contains('collapsed'));
      }}
      if (!flyout.classList.contains('collapsed')) {{
          focusDrawerSection(sectionKey || 'pilot');
      }}
  }};

  setNotamsFlyoutOpen = function(forceOpen) {{
      setUnifiedDrawerOpen(forceOpen, 'notams');
      if (typeof forceOpen !== 'boolean' || forceOpen) {{
          updateNotamsFlyoutStatus();
      }}
  }};

  setAirportCircleFlyoutOpen = function(forceOpen) {{
      setUnifiedDrawerOpen(forceOpen, 'airport');
  }};
  ...
  (function wirePilotFlyoutBehavior() {{
      var flyout = document.getElementById('pilotFlyout');
      var toggleBtn = document.getElementById('pilotToggleBtn');
      var closeBtn = document.getElementById('pilotFlyoutClose');
      if (!flyout || !toggleBtn) {{
          return;
      }}
  ...
  document.getElementById('clearPilotLatLon').onclick = function() {{
      var latInput = document.getElementById('pLat');
      var lonInput = document.getElementById('pLon');
      var pilotText = document.getElementById('currentPilotText');
      var ddText = document.getElementById('decimalDegreesText');
      ...
      if (pilotText) {{
          pilotText.textContent = 'Current Pilot Location: cleared (tap Get Pilot Location or enter coordinates)';
      }}
      if (ddText) {{
          ddText.textContent = 'Decimal Degrees: --, --';
      }}
      try {{
          localStorage.removeItem('pilotLocationDD');
          localStorage.removeItem('pilotLocationDMS');
          localStorage.removeItem('pilotLatitude');
          localStorage.removeItem('pilotLongitude');
          localStorage.removeItem('pilotLocationSource');
      }} catch (err) {{
          console.warn('[NOTAM MAP] unable to clear pilot location from localStorage', err);
      }}
      updatePilotLocationPromptState(null);
      if (latInput) {{
          latInput.focus();
      }}
  }};

  var centerPilotBtn = document.getElementById('centerPilot');
  if (centerPilotBtn) {{
      centerPilotBtn.onclick = function() {{
          centerMapToPilotRadius();
      }};
  }};

  var notamsFlyoutToggleBtn = document.getElementById('notamsFlyoutToggleBtn');
  if (notamsFlyoutToggleBtn) {{
      notamsFlyoutToggleBtn.onclick = function() {{
          setNotamsVisible(!notamsVisible);
      }};
  }}
  var showAllNotamDetailBtn = document.getElementById('showAllNotamDetailBtn');
  if (showAllNotamDetailBtn) {{
      showAllNotamDetailBtn.onclick = function() {{
          toggleAllNotamDetails();
      }};
  }}
    ...
    var queryPilotLocation = getPilotLocationFromQuery();
    ...
    linkedRealtimeMode = linkedRealtimeNotams.length > 0;
        ...
        if (queryPilotLocation) {{
            startupPilotLocation = queryPilotLocation;
        }} else if (restoredPilotLocation) {{
    ...
    (function buildAirportCircles() {{
        ...
        function buildAirportCircleDrawerContent(container) {{
            if (!container) return;
            container.innerHTML = '';

            var provinceRow = document.createElement('button');
            provinceRow.type = 'button';
            provinceRow.className = 'airport-circles-row';
            provinceRow.id = 'airportProvinceToggleRow';
            var provinceSwatch = document.createElement('span');
            provinceSwatch.style.width = '12px';
            provinceSwatch.style.height = '12px';
            provinceSwatch.style.borderRadius = '50%';
            provinceSwatch.style.background = '#1f3b63';
            provinceSwatch.style.border = '1px solid rgba(0,0,0,0.2)';
            var provinceLabel = document.createElement('span');
            provinceLabel.className = 'airport-circles-row-label';
            provinceLabel.textContent = 'Airports by Province';
            var provinceStatus = document.createElement('span');
            provinceStatus.className = 'airport-circles-row-status';
            provinceStatus.id = 'airportProvinceStatus';
            var updateProvinceRowStatus = function() {{
                var on = anySelectedAirportLayerVisible();
                provinceStatus.textContent = on ? 'On' : 'Off';
                provinceRow.style.opacity = on ? '1' : '0.55';
            }};
            provinceRow.appendChild(provinceSwatch);
            provinceRow.appendChild(provinceLabel);
            provinceRow.appendChild(provinceStatus);
            provinceRow.addEventListener('click', function() {{
                setSelectedAirportLayersVisible(!anySelectedAirportLayerVisible());
                updateAirportsToggleLabel();
                updateProvinceRowStatus();
            }});
            updateProvinceRowStatus();
            container.appendChild(provinceRow);

            circleTypes.forEach(function(type) {{
                var cfg = circleConfig[type];
                var row = document.createElement('button');
                row.type = 'button';
                row.className = 'airport-circles-row';
                ...
                label.className = 'airport-circles-row-label';
                label.textContent = cfg.label + ' (' + cfg.radiusNm + ' NM)';

                var status = document.createElement('span');
                status.setAttribute('data-role', 'status');
                status.className = 'airport-circles-row-status';
            ...
            window._updateAirportProvinceDrawerState = updateProvinceRowStatus;
        }}

        buildAirportCircleDrawerContent(document.getElementById('airportCircleRows'));
        ...
        window._airportCircleLegendControl = null;
    }})();
...
def normalize_output_html_head(output_path: Path, build_generated_at: str | None = None) -> None:
    try:
    ...
    build_stamp = build_generated_at or datetime.now().isoformat(timespec="seconds")
    generator_meta = (
        f'<meta name="generator" content="{SCRIPT_NAME} {SCRIPT_VERSION}" />\n'
        f'    <meta name="generator-release-date" content="{SCRIPT_RELEASE_DATE}" />\n'
        f'    <meta name="build-generated-at" content="{build_stamp}" />'
    )
    if '<meta name="generator" content="' not in text:
        text = text.replace(
            '<meta name="viewport" content="width=device-width, initial-scale=1.0" />',
            '<meta name="viewport" content="width=device-width, initial-scale=1.0" />\n    '
            + generator_meta,
...
def choose_output_path() -> Path:
    out_name = input(
        f"Output HTML filename [{DEFAULT_V5_OUTPUT_FILENAME}]: "
    ).strip()

    if not out_name:
        out_name = DEFAULT_V5_OUTPUT_FILENAME
    elif not out_name.lower().endswith(".html"):
        out_name += ".html"

    if out_name.lower() == LIVE_MAP_FILENAME.lower():
        print("\nLive map overwrite requested.")
        confirm = input(
            f"Type {OVERWRITE_CONFIRM_TOKEN} to proceed: "
        ).strip()
        if confirm != OVERWRITE_CONFIRM_TOKEN:
            ts = datetime.now().strftime("%Y%m%d_%H%M%S")
            out_name = f"NotamsMap_v5_candidate_{ts}.html"
            print(f"Confirmation mismatch. Writing candidate output instead -> {out_name}")

    return Path(out_name)


def backup_live_map_if_needed(output_path: Path) -> None:
    if output_path.name.lower() != LIVE_MAP_FILENAME.lower():
        return
    ...
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    backup_path = output_path.with_name(f"NotamsMap.backup.{ts}.html")
    shutil.copy2(output_path, backup_path)
    print(f"Live map backup created -> {backup_path.name}")


def main():
    build_generated_at = datetime.now().isoformat(timespec="seconds")
    print(f"Generator: {SCRIPT_NAME} {SCRIPT_VERSION} ({SCRIPT_RELEASE_DATE})")
    print(f"Build timestamp: {build_generated_at}")
    ...
    backup_live_map_if_needed(output_path)

    m.save(str(output_path))
    normalize_output_html_head(output_path, build_generated_at=build_generated_at)
