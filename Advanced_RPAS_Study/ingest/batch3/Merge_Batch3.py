#!/usr/bin/env python3
"""Select and merge up to 100 Batch 3 questions after duplicate screening against local questions.json.
...
def similarity(a,b):
    ...
    # A conservative combined score; exact-ish paraphrases are caught by either arm.
...
def main():
    ...
    ap.add_argument("--allow-shortfall", action="store_true",
                    help="Allow a real merge even if fewer than 100 survive screening")
    args = ap.parse_args()

    here = Path(__file__).resolve().parent
    repo = here.parents[1]
    qpath = repo / "data" / "questions.json"
    cpath = here / "Batch3_candidate_pool.json"
    audit_path = here / "Batch3_dryrun_audit.json"
    ...
    for cand in pool["questions"]:
        ...
        # Also compare against questions already selected in this batch.
        for old in selected:
    ...
    print(f"Batch 3 candidate pool: {len(pool['questions'])}")
    ...
    if len(selected) < sum(targets.values()) and not args.allow_shortfall:
        raise SystemExit("Refusing merge: fewer than 100 questions survived screening. Review Batch3_dryrun_audit.json or rerun with --allow-shortfall.")

    backup_dir = repo / "ingest" / "backups"
    backup_dir.mkdir(parents=True, exist_ok=True)
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    backup = backup_dir / f"questions_before_batch3_{stamp}.json"
    shutil.copy2(qpath, backup)

    data["questions"].extend(selected)
    data["totalQuestions"] = len(data["questions"])
    data["updated"] = date.today().isoformat()
    note = (f"{date.today().isoformat()} CHAT BATCH 3: duplicate-screened local ingest; "
            f"{len(selected)} TP 15263 core-Advanced questions selected from {len(pool['questions'])} candidates "
            f"across all 14 quiz categories.")
