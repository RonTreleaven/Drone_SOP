param(
    ...
    [string]$DayOfWeek = "",
    [string]$TaskDescription = "Refresh Drone SOP airspace data every 56 days and push changes to git."
)

$timePattern = '^(?:[01]\d|2[0-3]):[0-5]\d$'
if ($Time -notmatch $timePattern) {
    throw "Time must be in 24-hour HH:mm format, for example 06:10 or 18:30."
}

$scriptRoot = Split-Path -Parent $PSCommandPath
$runnerPath = Join-Path $scriptRoot "monthly_refresh_airspace_and_push.ps1"
