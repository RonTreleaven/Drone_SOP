...
#
# version 4.0 to create NotamsMap.html (v4) 
...
DEFAULT_V4_OUTPUT_FILENAME = "NotamsMapv4.html"
OVERWRITE_CONFIRM_TOKEN = "OVERWRITE_NOTAMSMAP"
SCRIPT_VERSION = "v4.0.1"
SCRIPT_RELEASE_DATE = "2026-05-13"
SCRIPT_NAME = "Gold v4 Notam Map Builder.py"
...
window.onload = function() {{
  ...
  airportsToggleCtl.onAdd = function() {{
      ...
      btn.id = 'airportsToggleBtn';
      btn.href = '#';
      btn.title = 'Airport circles panel';
      btn.setAttribute('aria-label', 'Airport circles panel');
      ...
      L.DomEvent.on(btn, 'click', function(ev) {{
          ...
          if (typeof setUnifiedDrawerOpen === 'function') {{
              setUnifiedDrawerOpen(true, 'airport');
              return;
          }}
  ...
  notamsToggleCtl.onAdd = function() {{
      ...
      var btn = L.DomUtil.create('a', 'left-mini-toggle-btn', d);
      btn.id = 'notamsToggleBtn';
      btn.href = '#';
      btn.title = 'NOTAM tools';
      btn.setAttribute('aria-label', 'NOTAM tools');
      btn.setAttribute('aria-haspopup', 'dialog');
      btn.setAttribute('aria-expanded', 'false');
      ...
      L.DomEvent.on(btn, 'click', function(ev) {{
          L.DomEvent.stop(ev);
          if (typeof setUnifiedDrawerOpen === 'function') {{
              setUnifiedDrawerOpen(true, 'notams');
              return;
          }}
  ...
  function setPilotFlyoutCollapsed(collapsed) {{
      ...
      var host = flyout ? flyout.parentElement : null;
      ...
      flyout.style.display = collapsed ? 'none' : 'block';
      if (host) {{
          host.style.width = collapsed ? '0px' : '';
          host.style.height = collapsed ? '0px' : '';
          host.style.overflow = collapsed ? 'hidden' : 'visible';
      }}
...
def choose_output_path() -> Path:
    out_name = input(
        f"Output HTML filename [{DEFAULT_V4_OUTPUT_FILENAME}]: "
    ).strip()

    if not out_name:
        out_name = DEFAULT_V4_OUTPUT_FILENAME
    ...
    if out_name.lower() == LIVE_MAP_FILENAME.lower():
        ...
        if confirm != OVERWRITE_CONFIRM_TOKEN:
            ...
            out_name = f"NotamsMap_v4_candidate_{ts}.html"
