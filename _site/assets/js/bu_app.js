// assets/js/app.js
// v1.0 RLT May 22, 2025

document.addEventListener('DOMContentLoaded', () => {
  // Choose the right JSON path: ./data on root pages; ../data for /sections/*
  ...
  fetch(jsonPath)
    ...
    .catch(err => {
      ...
      if (sel) sel.textContent = 'Error loading sections — see console';
    });
});
...
function renderSummary(sections) {
  ...
  // Utility functions for dynamic naming
  function getDateStr() {
  ...
  document.getElementById('export-csv').addEventListener('click', () => {
    // Build CSV: flightLog header + checklist
    let csv = 'Field,Value\n'
      + `Date,${flightLog.date||''}\n`
